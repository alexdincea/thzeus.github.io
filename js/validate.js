(function($) {
  ...
  Validate.fn = $.Validate.prototype = {
    ...
    setCallback: function(type, e, data) {
      ...
      if (events && typeof events[type] != "undefined") {
        ...
        for (var i = 0; i < len; i++) {
          ...
          if (
            namespace == "tools." + $.Validate.NAME ||
            namespace == $.Validate.NAME + ".tools"
          ) {
    ...
    showError: function($el, name, text) {
      $("#" + name + "-error")
        .addClass(this.opts.spanClassName)
        .html(text)
        .show();
    ...
    showTooltip: function($el, text) {
      ...
      if (size !== 0) {
        ...
        var tooltip = $(
          '<div class="validate-tooltip tooltip tooltip-theme-red" />'
        ).html(text);
        tooltip.css({
    ...
    clear: function() {
      this.$element
        .find("." + this.opts.errorClassName)
        .removeClass(this.opts.errorClassName);
      $(".validate-error")
        .removeClass("validate-error")
        .html("")
        .hide();
